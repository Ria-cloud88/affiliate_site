...
# ■制約（絶対厳守）
【文字数】 絶対に2500～3000文字（それ以上は書かない）
【見出し】 h2は最大4個。各h2の説明は300～400文字程度
【キーワード】 記事のメインキーワードを本文に3～5回自然に含める
【句点】 「。」で文を終わらせる。改行は多用しない

# ■記事構成（これ以上複雑にしない）
1. # タイトル
2. 導入（読者の悩みや疑問を1～2文で示す）
3. ## 見出し1（最初の答え・概要）
4. ## 見出し2（詳しい説明）
5. ## 見出し3（実践例・注意点など）
6. ## まとめ（CTA付き）
...
# ■書き方のルール
・短い文を意識。1文は最大30字程度
・「です」「ます」で統一。「ある」「いる」との混在禁止
・同じ表現を繰り返さない。言い換えを使う
・「また」「そして」「しかし」で文を始めない
・「〜と思います」「〜だと考えられます」は使わない
・読点「、」は1文に1～2個まで

# ■完全に禁止する表現
・「いかがでしたか」「ぜひご覧ください」などAIの典型表現
・「重要です」「大切です」「できます」の繰り返し
・「このように」「このため」「その結果」などの説明臭い接続詞
・「〜といえます」「〜と言えるでしょう」という曖昧さ
・「注目されています」「話題になっています」などのジャーナリズム風
・難しい言葉。小学生でも理解できる日本語

# ■出力形式
・Markdown形式のみ
・絶対に3000文字以上は書かない
・前置きなし。最初の行は # タイトル から開始"""
