...
【記事終了ルール】
・「## まとめ」の後に、追加の見出しは一切生成しない
・まとめの後は即座に記事を終了
・「## 関連記事」「## 補足」などの余分な見出しを作らない
...
def fix_article_content(content: str) -> str:
    """記事の構造を修正: ## 目次【非表示】を削除、## まとめ後の見出しを修正"""
    lines = content.split('\n')
    fixed_lines = []
    i = 0
    ...
    while i < len(lines):
        ...
        if '目次【非表示】' in line:
            # この行をスキップ
            i += 1
            # 目次の内容（ol/ulブロック）をスキップ
            while i < len(lines):
                if lines[i].strip().startswith('<') or lines[i].strip().startswith('1.') or lines[i].strip().startswith('-'):
                    i += 1
                elif lines[i].strip() == '':
                    i += 1
                else:
