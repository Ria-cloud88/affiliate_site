...
def fix_article_content(content: str) -> str:
    """記事の構造を修正: ## 目次【非表示】→ ## 目次、## まとめセクションの段落をHTML保護"""
    # "## 目次【非表示】" を "## 目次" に修正
    ...
    fixed_lines = []
    i = 0
    protect_next_paragraph = False

    while i < len(lines):
        line = lines[i]

        # "## まとめ" を検出したら、次の段落をHTMLコメントで保護するフラグを設定
        if line.strip().startswith('## ') and 'まとめ' in line:
            fixed_lines.append(line)
            protect_next_paragraph = True
            i += 1
        # 保護フラグが立っていて、空行ではない行（段落の最初の行）にHTMLコメントを挿入
        elif protect_next_paragraph and line.strip() and not line.strip().startswith('## '):
            # 段落の最初の文字の前にHTMLコメントを挿入（パーサーの誤認識防止）
            fixed_lines.append('<!-- preserve-paragraph -->' + line)
            protect_next_paragraph = False
            i += 1
        # 空行はそのまま通す
        elif not line.strip():
