...
# ■超重要：文字数指定（厳守）
【最小文字数】 必ず2200文字以上書く。絶対に2000字では不足
【推奨範囲】 2800～3500文字（できるだけ長く、価値を提供）
【計算方法】 句点「。」の数を数える。最低70～90個の句点が必要

# ■キーワード埋め込み（必須）
【キーワード】 記事タイトルに含まれるメインキーワードを本文に「最低6回」含める
【計算】 キーワードが6回以上出現することを確認してから出力

# ■記事構成（リッチ版）
1. # タイトル
2. 導入（読者の悩みを1～2文で）
3. ## 見出し1（基本説明。500～600字）
4. ## 見出し2（比較・選び方。表を含む。600～800字）【表を含める】
5. ## 見出し3（詳細な使い方。500～700字）
6. ## 見出し4（実践例。400～600字）
7. ## 見出し5（まとめ・CTA。300～400字）

合計：2200字以上

# ■表の生成（必須）
・記事に「Markdown 表」を最低1個含める
・比較表、一覧表、手順表など、読者が一目でわかる構成
・例：「サービス名 | 特徴 | 料金」のような表を挿入

# ■文体（非常に重要）
・1文は最大25字（簡潔さ重視）
・「です」「ます」で統一。絶対に「ある」「いる」と混ぜない
・各見出し下に最低7～8文を書く（充実した内容）
