...
9. ## 見出し7（まとめ・CTA。300～400字。次のアクション提示）
...
【改善例】
❌ 「このツールは非常に便利であり、多くの人に活用されています。できるだけ早く始めることをお勧めします」
✅ 「このツールは45分の作業を15分に短縮できます。月額300円で利用できるため、試す価値があります」

【文体ルール】
・1文は15～25字が目安。30字を超えたら分割する
・「です」「ます」で統一。「ある」「いる」と混ぜない
・具体的な数字・データを使う（「多い」ではなく「3倍」）
・感情や推測を避け、事実のみを述べる
・同じ単語を繰り返さず、同義語で言い換える
・専門家の口調：簡潔・直接的・証拠ベース
・読者に行動を促す（「試す」「確認する」「調べる」など）
...
・前置きなし。「# タイトル」から開始
・各見出し下に画像埋め込み予定：![説明](/images/blog/XXXX.jpg)
・末尾に余分なテキスト・メタデータなし
...
def generate_table_of_contents(content: str) -> str:
    ...
    # 3500字相当（句点150個以上）の場合のみ目次を生成
    if period_count < 150 or len(headings) < 7:
        return content
    ...
    if insert_position > 0:
        toc_text = '\n\n' + '\n'.join(toc_lines) + '\n\n'
        content = content[:insert_position] + toc_text + content[insert_position:]
...
"""
    ...
    keywords_section = generate_related_keywords(main_kw, related_kws)

    # フルコンテンツを組み立て
    full_content = frontmatter + article_body + related_articles + popular_articles + keywords_section
